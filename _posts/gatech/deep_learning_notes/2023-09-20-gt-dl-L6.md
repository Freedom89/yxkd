...
Now that we have the gradients with respect to our weights, let's calculate the gradients with respect to our input. Teh reason we need this is not to update the weights for this layer, but to pass back to whatever previous layer occurred before this one. 
...
* ReLU instead of isgmoid or tanh
...
As you can see when the example goes from 10million to 300 million the MAP continues to improve.

Active research is still happening when you have low-labeled settings:
